...
import config
from llm_from_scratch.GPT.generate import generate_loop
from llm_from_scratch.GPT.gpt_model import GPTModel
from llm_from_scratch.utils import alpaca_prompt_format, ids_to_text, text_to_ids
...
with open(config.instruct_test_path, "r") as f:
    test_txt = json.load(f)

model_cfg = config.config_creator("gpt_m")
model = GPTModel(model_cfg)
# loading our finetuned model params
ft_checkpoint = torch.load(config.ft_instruct_w_gpt2, weights_only=False)
